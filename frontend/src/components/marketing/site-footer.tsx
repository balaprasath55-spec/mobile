...
export function SiteFooter() {
  ...
  return (
    <footer className="border-t border-navy/5 bg-navy text-white dark:border-white/10">
      ...
      <div className="mx-auto grid max-w-6xl gap-8 px-4 py-12 sm:px-6 sm:py-14 md:grid-cols-4">
        <div>
          <p className="font-display text-xl font-semibold">
            MR <span className="text-accent-300">Mobile</span> Zone
          </p>
          <p className="mt-3 text-sm text-white/70">{SITE.tagline}</p>
          <p className="mt-4 break-words text-sm text-white/60">{SITE.address}</p>
          <p className="mt-2 text-sm text-white/60">{SITE.phoneDisplay}</p>
          <p className="mt-2 break-all text-sm text-white/60">{SITE.email}</p>
        </div>
        {columns.map((col) => (
          <div key={col.title}>
            ...
            <ul className="mt-3 space-y-1">
              {col.links.map((link) => (
                <li key={link.href}>
                  <Link
                    href={link.href}
                    className="inline-block py-2 text-sm text-white/60 transition hover:text-white"
                  >
                    {link.label}
                  </Link>
                </li>
      ...
      <div className="border-t border-white/10 px-4 py-5 pb-[max(1.25rem,env(safe-area-inset-bottom))] text-center text-xs text-white/50">
