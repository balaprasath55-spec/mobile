...
import type { DemoEnquiry, DemoEnquiryStatus } from "@/lib/demo-store";
...
export default async function EnquiriesAdminPage({
  ...
  const data = await serverApi<{ enquiries: DemoEnquiry[] }>("/api/enquiries");
  const rows = data.enquiries.filter((e) =>
    status && statuses.includes(status) ? e.status === status : true
  );
